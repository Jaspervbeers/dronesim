...
class controller:
    ...
    def __init__(self, signMask, noiseBlock, PIDGainPath, PIDGainFile = None):
        ...
        if PIDGainFile is None:
            PIDGainFile = 'droneNDIGains.json'
        ...
        self.gains_inner = np.array(np.matrix(gainDict['VirtualControl']['P'])).reshape(-1)
    ...
    def extractPIDsFromJSON(self, PIDJSON):
        ...
        for col in range(P.shape[1]):
            PID.append(PIDController.PID(P[:, col].reshape(-1, 1), I[:, col].reshape(-1, 1), D[:, col].reshape(-1, 1), refreshIntegrator=True))
        return PID
    ...
    def control(self, simVars):
        ...
        self.outerLoop() # Sets attitude command
        self.innerLoop() # Sets rates and thrust -> converts to rotor eRPM 
        self.commandedOmega = self.commandedOmega_NDI
    ...
    def innerLoop(self):
        ...
        #   NOTE: Outerloop sets vel E command
        self.w = self.vel_E[:, 2]
        ...
        self.commandedInput = np.matmul(np.linalg.inv(self._controlEffectiveness()), (self.virtualInput - self._fx()))
        ...
        self.commandedOmega_NDI = np.sqrt(commandedOmega2)
    ...
    def outerLoop(self):
        ...
        accRef_E = self._PIDLoop(velRef_E, self.vel_E, self.velAccPID, self.dt)
        
        # Virtual input
        gains_ALT = np.array([1, 1, 1])
        virtualInput_ALT = -1*gains_ALT*vel_E_error + accRef_E
    ...
    def _fx(self):
        att = self.state[:, :3]
        rates = self.state[:, 6:9]
        vel = self.state[:, 3:6]
        # vel_dot = self._getVelDot(att, rates, vel)
        vel_dot = self._getVelDot_E(att, rates, vel)
        rates_dot = self._getRateDot(rates)
        return np.hstack((rates_dot.reshape(3), vel_dot.reshape(3)[-1]))


    def _getVelDot(self, att, rates, vel):
        return QuatRot(Eul2Quat(att), np.array([0, 0, self.droneParams['g']]).reshape(1, -1), rot = 'E2B') - np.cross(rates, vel)
    ...
    def _getRateDot(self, rates):
        Iv = self.droneParams['Iv']
        inv_Iv = np.linalg.inv(Iv)
        return -1*np.matmul(inv_Iv, np.cross(rates, np.matmul(Iv, rates.T).T).T)
    ...
    def FM2OmegaMapping(self):
        ...
        if self.simVars['model']._simplifiedModel is None:
            ...
            # Take w_2 variants as controller uses NDI Moments -> w^2 (rotor speed squared)
    ...
    def _saturate(self, a, amin, amax):
        a[a < amin] = amin
        a[a > amax] = amax
        return a
